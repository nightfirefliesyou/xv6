...
void  
vmprint1(pagetable_t pagetable,int vmprintnum){
  if(vmprintnum==0)
  printf("page table %p\n",pagetable);
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
      for(int tmp=0;tmp<vmprintnum;tmp++){
        printf(".. ");
      }
      uint64 child = PTE2PA(pte);
      printf("..%d: pte %p pa %p\n",i,pte,child);
      if((pte & (PTE_R|PTE_W|PTE_X)) == 0)
      vmprint1((pagetable_t)child,vmprintnum+1);
    }
    }
  }
...
int
userkvmcopy(pagetable_t old, pagetable_t new, uint64 begin, uint64 end)
{
  pte_t *pte, *newPte;
  uint64 pa, i;
  uint flags;
  ...
  for(i = PGROUNDUP(begin); i < end; i += PGSIZE){
    if((pte = walk(old, i, 0)) == 0)
      panic("uservmcopy pte not exist");
    if ((newPte = walk(new, i, 1)) == 0)
      panic("uservmcopy newpte not exist");
    pa = PTE2PA(*pte);
